...
- **선행 Task**: FE-1
- **완료 조건**:
  - [ ] 회원가입 성공 시 로그인 페이지로 이동한다 (FR-1.1)
  - [ ] 이메일 중복 응답 시 중복 안내 메시지가 화면에 표시된다 (EX-5)
  - [ ] 로그인 성공 시 토큰/사용자 정보가 authStore에 저장되고 역할별 메인 화면으로 이동한다 (FR-1.2)
...
- **선행 Task**: FE-2
- **완료 조건**:
  - [ ] 비로그인 상태로 보호 경로 접근 시 로그인 페이지로 리다이렉트된다 (EX-4, FR-1.3)
  - [ ] buyer 계정으로 관리자 경로 접근이 차단된다
  - [ ] 로그아웃 시 스토어가 초기화되고 로그인 페이지로 이동한다 (FR-1.5)
...
- **선행 Task**: FE-3
- **완료 조건**:
  - [ ] 진행/예정 프로모션 카드 목록이 렌더링된다 (FR-3.1)
  - [ ] 카드 클릭 시 상세 페이지로 이동한다
  - [ ] 로딩/에러/빈 목록 상태가 각각 화면에 표시된다
...
- **선행 Task**: FE-4
- **완료 조건**:
  - [ ] 신청 성공 시 신청 완료 상태로 화면이 갱신된다 (FR-3.2)
  - [ ] 이미 신청한 프로모션에서 중복 신청이 차단되고 안내 메시지가 표시된다 (EX-1)
  - [ ] 종료/미게시 프로모션에서 신청 버튼이 비활성화되고 서버 거부 메시지도 표시된다 (EX-2)
...
- **선행 Task**: FE-5
- **완료 조건**:
  - [ ] 신청 응답의 당첨 경품이 애니메이션 종료 후 표시된다 (FR-3.2)
  - [ ] 새로고침/재진입 시에도 서버가 준 동일 결과가 표시되고 재추첨이 발생하지 않는다 (BR-2)
  - [ ] 클라이언트 코드에 추첨 로직(랜덤 선택)이 없다
...
- **선행 Task**: FE-5
- **완료 조건**:
  - [ ] 신청 목록에 상태와 당첨 경품명이 표시된다 (FR-3.4)
  - [ ] 취소 후 목록이 즉시 갱신되어 상태가 "취소"로 표시된다 (FR-3.3)
  - [ ] 취소 건을 재신청하면 상태가 다시 "신청"으로 표시된다 (BR-1)
...
- **선행 Task**: FE-3
- **완료 조건**:
  - [ ] 상태별로 게시/종료 버튼이 조건부 노출된다 (BR-4)
  - [ ] 게시/종료 후 목록 상태가 갱신된다 (FR-2.4)
  - [ ] 경품 없는 게임 프로모션 게시 시도 시 서버 거부 메시지가 표시된다 (EX-3)
...
- **선행 Task**: FE-8
- **완료 조건**:
  - [ ] 등록 성공 시 상태 "예정"으로 목록에 나타난다 (FR-2.1, BR-4)
  - [ ] 게임 적용 체크 시에만 경품 입력 영역이 노출되고 저장된다 (FR-2.2)
  - [ ] 기존 프로모션 수정 시 기존 값이 폼에 채워지고 저장 후 반영된다 (FR-2.3)
...
- **선행 Task**: FE-8
- **완료 조건**:
  - [ ] 신청자 목록이 표 형태로 표시된다 (FR-2.5)
  - [ ] 당첨 경품이 없는 건은 빈 값/하이픈으로 표시된다
  - [ ] 신청 0건일 때 빈 상태 메시지가 표시된다
...
- **선행 Task**: FE-3
- **완료 조건**:
  - [ ] 이름/거래처명 수정 후 저장하면 변경값이 반영된다 (FR-4.1)
  - [ ] 비밀번호 변경 시 현재 비밀번호 오류가 화면에 표시된다 (FR-4.2)
  - [ ] 비로그인 접근 시 로그인 페이지로 리다이렉트된다 (EX-4)
...
- **완료 조건**:
  - [ ] 모바일 폭(약 375px)과 데스크탑에서 전 페이지가 가로 스크롤 없이 표시된다
  - [ ] `4-user-scenario.md`의 시나리오 1~8이 처음부터 끝까지 오류 없이 수행된다
  - [ ] EX-1~5 각각에 대해 사용자에게 원인을 알리는 메시지가 화면에 표시된다
